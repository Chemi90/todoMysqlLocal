...
class medoo
{
		...
		try {
			...
			switch ($type)
			{
				...
				case 'mysql':
					...
					{
						$dsn = $type . ':host=' . $this->server . ($is_port ? ':' . $port : '') . ';dbname=' . $this->database_name;
					}
				...
				if (isset($match[ 4 ]))
				{
					...
					if ($operator == '!')
					{
						switch ($type)
						{
							...
							case 'string':
								...
								//$wheres[] = $column . ' != ' . $this->fn_quote($key, $value). ' COLLATE utf8_spanish_ci';
								break;
						}
					}
					...
					if ($operator == '~' || $operator == '!~')
					{
						...
						foreach ($value as $item)
						{
							...
							//$like_clauses[] = $column . ($operator === '!~' ? ' NOT' : '') . ' LIKE ' . $this->fn_quote($key, $item). ' COLLATE utf8_spanish_ci';
						}
	...
	protected function where_clause($where)
	{
		...
		if (is_array($where))
		{
			...
			if (!empty($where_AND))
			{
				...
				if (is_string($where[ $value[ 0 ]])) {
					$where_clause = ' WHERE ' . $where[ $value[ 0 ]];
				} else {
			...
			if (isset($where[ 'ORDER' ]))
			{
				...
				if (is_array($ORDER))
				{
					...
					foreach ($ORDER as $column => $value)
					{
						...
						else if (strtoupper($value) === 'ASC' || strtoupper($value) === 'DESC')
						{
	...
	protected function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
	{
		...
		//echo 'SELECT ' . $column . ' FROM ' . $table_query . $this->where_clause($where);
	...
	public function insert($table, $datas)
	{
		...
		foreach ($datas as $data)
		{
			...
			//echo 'INSERT INTO ' . $this->table_quote($table) . ' (' . implode(', ', $columns) . ') VALUES (' . implode(', ', $values) . ')';
			if (count($datas) === 1) {
				return $res;
			}
	...
	public function update($table, $data, $where = null)
	{
		...
		//echo 'UPDATE ' . $this->table_quote($table) . ' SET ' . implode(', ', $fields) . $this->where_clause($where);
		return $this->exec('UPDATE ' . $this->table_quote($table) . ' SET ' . implode(', ', $fields) . $this->where_clause($where));
	}

	public function delete($table, $where)
	{
		//echo 'DELETE FROM ' . $this->table_quote($table) . $this->where_clause($where);
	...
	public function get($table, $join = null, $columns = null, $where = null)
	{
		...
		//echo $this->select_context($table, $join, $columns, $where) . ' LIMIT 1';
	...
	{
		//echo $this->select_context($table, $join, $column, $where, 'COUNT');
